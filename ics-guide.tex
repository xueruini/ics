...
This section describes the commands used in the prolog of your paper. The prolog
...
\label{sec:structure-document}
We require the material appearing between \verb|\begin{document}| and
  \verb|\end{document}| to be displayed over two columns. Furthermore, the
structure of the article must be as follows: 

\begin{itemize}
\item Text of the paper
\item Acknowledgment
...
This section describes the commands used to produce the title page of your 
paper.
...
\address{%
  ...
  \and
  $^{2}$Tsinghua University Press, Beijing 100084, China}
...
two commands, list each author's email. Do not use superscripts
to group emails.
...
the top. We recommend not to use vertical borders in tables if possible, but this is
not mandatory.

Both figures and tables accept the \texttt{h, t, b, p} options. If the figures
or tables are too big to fit in one column, please use their corresponding star
forms (\texttt{figure*} and \texttt{table*}) to place figures and tables over
two columns.
